...
export const webhookEvents = pgTable(
  ...
  (table) => [unique("webhook_events_signature_unique").on(table.signature)]
);
...
export const indexTypes = pgTable(
  "index_types",
  {
    id: varchar().primaryKey().notNull(),
    name: varchar().notNull(),
  ...
  (table) => [unique("index_types_name_unique").on(table.name)]
);
...
export const indexSubscriptions = pgTable(
  "index_subscriptions",
  {
    id: varchar().primaryKey().notNull(),
    userId: varchar("user_id").notNull(),
    ...
    addresses: jsonb().default([]).notNull(),
